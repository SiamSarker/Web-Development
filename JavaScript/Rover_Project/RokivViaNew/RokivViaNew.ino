...
<body>
  ...
  <div id="chart-gps" class="container"></div>
</body>
<script>
  ...
  setInterval(function ( ) {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        var x = (new Date()).getTime(),
            y = parseFloat(this.responseText);
            document.getElementById("accelerometer").innerHTML = this.responseText;
...
void setup(){
  ...
  server.on("/gps", HTTP_GET, [](AsyncWebServerRequest *request){
    request->send_P(200, "text/plain", String(gps).c_str());
  });
  // Start server
  server.begin();
}
